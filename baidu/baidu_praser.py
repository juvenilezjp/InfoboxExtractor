...
from baidu.exception import *

CLASS_TITLE = ['lemmaWgt-lemmaTitle-title']
CLASS_DISAMBIGUATION = ['polysemant-list']
...
CLASS_INFOBOX = ['basicInfo-item name','basicInfo-item value']


class Page(object):
    def __init__(self, string, local_html = None, encoding='utf-8'):
    ...
    def get_infobox(self):
        ...
        for tag in self.soup.find_all(class_=CLASS_INFOBOX[0]):
            infobox_name.append(tag.get_text())

        for tag in self.soup.find_all(class_=CLASS_INFOBOX[1]):
            infobox_value.append(tag.get_text())

        if len(infobox_name) != len(infobox_value):
            print('异常')
        return infobox_name,infobox_value

    def get_title(self):

        title_name = []

        for tag in self.soup.find_all(class_=CLASS_TITLE):
            title_name.append(tag.get_text())
